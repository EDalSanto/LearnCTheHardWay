...
static char *test_operations() {
  ...
  mu_assert(make_random(map), "Didn't mae randome fake radix map.");

  RadixMap_sort(map);
  mu_assert(check_order(map), "Failed to properly sort the RadixMap.");
  mu_assert(test_search(map), "Failed the search test.");
  mu_assert(check_order(map), "RadixMap didn't staty sorted after search.");
  ...
  while (map->end > 0) {
    ...
    // test that the end if now the old value but uint32 max so it trails off
